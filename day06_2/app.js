...
app.get("/todo/detail", async (req,res)=>{
    try {
        ...
        res.render("todolist/TodoDetail", {todo: fetch});
      } finally {
        await client.close();
      }
});
...
app.get("/todo/detail", async (req, res) => {
    try {
        ...
        // 요청으로부터 전달된 _id 값을 이용하여 특정 항목을 조회
        const query = { _id: new ObjectId(req.query._id) };
        const todo = await collection.findOne(query);

        if (todo) {
            // 조회된 항목이 있다면 detail 페이지로 렌더링
            res.render("todolist/TodoDetail", { todo: todo });  // ejs 파일에 todo 객체 전달
        } else {
            // 항목이 없으면 목록 페이지로 리다이렉트
            console.log("존재하지 않는 todo 항목입니다.");
            res.redirect("/todo/list");
        }
    } catch (err) {
        console.error(err);
